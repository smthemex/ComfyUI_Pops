...
def get_named_beta_schedule(
    schedule_name, num_diffusion_timesteps, linear_start=0.0001, linear_end=0.02
):
    """
    ...
    if schedule_name == "linear":
        ...
        beta_start = scale * linear_start  # * 0.00085
        beta_end = scale * linear_end  # 0.012
        return np.linspace(
...
class GaussianDiffusion:
    ...
    def p_sample(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, generator=None
    ):
        """
        ...
        noise = th.randn(x.size(), dtype=x.dtype, layout=x.layout, device=x.device, generator=generator)
        nonzero_mask = (
